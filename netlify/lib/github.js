  ...
  async accessToken() {
    ...
    if (!response.ok) {
      /*
        ...
        GitHub objected to, and the three identifiers say what we asked with —
        which is what found the last fault: an installation ID holding a settings
        URL rather than a number. The private key, the JWT, the token, the
        Authorization header and the response body are never touched.
