...
if (exists("dist")) {
  // Standard event pages ARE generated as of Phase 11 and the Business Forum as
  // of Phase 12. The listing and the homepages are not.
  ...
  assert(created.length === 0,
    "no events listing or homepage has been generated",
...
if (!exists("dist")) {
  ...
  if (evMissingPages.length === 0) {
    ...
    const forbidden = ["dist/events.html", "dist/pl/events.html",
      "dist/index.html", "dist/pl/index.html"];
    const created = forbidden.filter((p) => exists(p));
    assert(created.length === 0,
      "no events listing or homepage was generated by this phase",
...
if (bfRec) {
  ...
  const BF_LOC_REQUIRED = ["title", "hero_eyebrow", "back_link", "back_link_bottom",
    ...
    "card_title", "card_summary", "card_image_alt", "flagship_tag",
...
if (!exists("dist")) {
  note("dist/ not built — run `npm run build` to validate the generated Forum pages");
} else {
  const BF_PAGES = [
  ...
  /* -- public files: only the justified css/pbf.css change is permitted ----- */
  {
    ...
    const untouched = gitDiff(["status", "--porcelain", "--",
      "css/style.css", "js/main.js", "assets", "sitemap.xml", "robots.txt", "netlify.toml",
      "index.html", "pl/index.html", "events.html", "pl/events.html",
      "event-business-forum.html", "pl/event-business-forum.html",
      "content/announcements"]);
    if (untouched === null) {
      note("git unavailable — public-file guard skipped");
    } else {
      const changed = untouched.split("\n").map((l) => l.trim()).filter(Boolean);
      assert(changed.length === 0,
        "every public file except css/pbf.css is untouched (live pages, style.css, main.js, assets, sitemap, netlify)",
    ...
    if (pbfDiff === null) {
      note("git unavailable — css/pbf.css scope guard skipped");
    } else if (pbfDiff.trim() === "") {
      note("css/pbf.css has no uncommitted change (already committed, or unchanged)");
    } else {
